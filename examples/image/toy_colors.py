...
def rule1_score(model, X):
    return np.mean(
        [fraction_inside_corners(grad) for grad in largest_mag_2d(model.input_gradients(X))])


def rule2_score(model, X):
    return np.mean(
        [fraction_inside_topmids(grad) for grad in largest_mag_2d(model.input_gradients(X))])
