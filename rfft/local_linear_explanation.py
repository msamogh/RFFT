...
class LocalLinearExplanation():
    ...
    def imshow_bw(self, imgshape, cutoff=0.0, xoff=0, yoff=0, size=75, lw=2, pwr=2):
        # display the image
        l, w = imgshape
        plt.imshow(self.x.reshape(imgshape), interpolation='none', cmap='gray', extent=[w*xoff, w*(xoff+1), l*yoff, l*(yoff+1)])
                ...
                for i, coef in enumerate(listwrap(coefs[y][x])):
                    if abs(coef) > maxcoef * cutoff:
                        ...
                        plt.scatter(xx, yy, s=size, lw=lw,
                                    # make opacity relative to magnitude
                                    alpha=(abs(coef)/maxcoef)**pwr,
...
def image_grid(images, imgshape, length=None, **kwargs):
    fauxpls = [LocalLinearExplanation(
        images[i], 0, np.zeros_like(images[i])) for i in range(len(images))]
