...
def to_logprob(prob):
    return prob - tf.reduce_logsumexp(prob, axis=1, keep_dims=True)
...
class TensorflowPerceptron(Perceptron):
    ...
    def optimizer(self, l2_grads=1000, l1_grads=0, l2_params=0.0001, learning_rate=0.001):
        ...
        return optimizer.minimize(
            self.loss_function(l2_grads=l2_grads, l1_grads=l1_grads, l2_params=l2_params))
    ...
    def fit(self, X, y, A=None,
        ...
        with tf.Session() as sess:
            ...
            for i in range(num_epochs * num_batches):
                idx = slice((i % num_batches) * batch_size,
                            ((i % num_batches) + 1) * batch_size)
                sess.run(optimizer, feed_dict={
                    self.X: X[idx], self.y: y[idx], self.A: A[idx]})
