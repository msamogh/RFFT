...
def word_rows(columns, cutoff=None, startafter=None, **kwargs):
    ...
    for row in range(len(columns[0])):
        ...
        for col in range(len(columns)):
            ...
            if scales:
                html += word_tags(*columns[col][row],
                                  scale=scales[col], **kwargs)
            else:
...
other_label = {'alt.atheism': 'soc.religion.christian',
...
def email_html(email, row, title=None, swatches=True, power=2):
    ...
    for wt, w in poswts:
        email = re.sub(r"([^a-zA-Z0-9]){}([^a-zA-Z0-9])".format(w),
                       "\\1{}\\2".format(
                           "<span style='background: rgba(155,217,230,{});'>{}</span>".format((wt/maxwt)**power, w)),
                       email)
    for wt, w in negwts:
        email = re.sub(r"([^a-zA-Z0-9]){}([^a-zA-Z0-9])".format(w),
                       "\\1{}\\2".format(
                           "<span style='background: rgba(238,125,50,{})'>{}</span>".format((-wt/maxwt)**power, w)),
                       email)
    email = email.replace('\n', '<br>')
    swatch1 = ''
    swatch2 = ''
    if swatches:
        ...
        swatch2 = '<span style="background: rgb(238,125,50); font-family:monospace; padding: 0 5px; margin-left: 5px;">+{}</span>'.format(
            other_label[pred])
